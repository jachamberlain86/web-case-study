...
export default function IngredientList() {
  ...
  const updateIngredientCards = (ingredientsArr) => {
    ...
    const searchFilteredArr = search.length ? filterBySearch(clonedArr) : clonedArr;
  ...
  return (
    <div>
      <div className='ingredient-list__filter-container'>
        <input type="text" placeholder='Search...' onChange={(event) => handleSearchInput(event)} className='ingredient-list__search-bar'/>
        <div className='ingredient-list__button-container'>
          <button onClick={() => handleFilterButton('vegetarian')} className={ vegetarianActive ? 'ingredient-list__filter-button--active' : 'ingredient-list__filter-button' }>Vegetarian</button>
          <button onClick={() => handleFilterButton('vegan')} className={ veganActive ? 'ingredient-list__filter-button--active' : 'ingredient-list__filter-button' }>Vegan</button>
          <button onClick={() => handleFilterButton('local')} className={ localActive ? 'ingredient-list__filter-button--active' : 'ingredient-list__filter-button' }>Local</button>
          <button onClick={() => handleFilterButton('bio')} className={ bioActive ? 'ingredient-list__filter-button--active' : 'ingredient-list__filter-button' }>Bio</button>
        </div>
      </div>
      <div className='ingredient-list__main-container'>
        {ingredientCards}
      </div>
    </div>
  )
