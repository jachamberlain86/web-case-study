...
export default function IngredientCard({ingredient}) {
  ...
  tags.forEach((tag) => {
    if (ingredient[tag]) {
      const badge = <div key={tag} className='ingredient-card__badge'>{tag}</div>
      badges.push(badge)
    }
  })
  ...
  return (
    <div className='ingredient-card__main-container'>
      <img src={`${REACT_APP_DIGITALOCEAN_SPACE_ALIMENTS}/${ingredient.picture}`} alt={ingredient.name} className='ingredient-card__image'/>
      ...
      <div className='ingredient-card__badge-container'>
      {badges}
      </div>
      <div className='ingredient-card__date-container'>

      <p className='ingredient-card__body--date '>Created on {creationDate}</p>
      </div>
    </div>
  )
}
